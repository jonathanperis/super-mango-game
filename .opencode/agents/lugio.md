...
**IMPORTANT:** The `description` field is a TOML multi-line string (`""" ... """`). It is the machine-readable copy of the comment block. The editor serializer preserves this field on save — without it, the description is lost when the editor re-serializes the file. **Always write both the comment block AND the description field.** Attribution goes ONLY in the `generated_by` scalar field — never inside the description. (See Lessons Learned #1.)
